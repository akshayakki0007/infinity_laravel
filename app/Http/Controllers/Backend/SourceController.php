...
class SourceController extends Controller
{
    ...
    {       
        ...
        $this->ModuleTitle = 'Source';
        $this->ModuleView  = 'Backend/source.';
        $this->ModulePath  = 'admin/source';
    }
    ...
    public function store(Request $request)
    {
        ...
        if($arrModel->save()) 
        {
            ...
            $this->JsonData['msg']    = 'Source added successfully.';
        }
        else
        {
            DB::rollBack();
    ...
    public function edit($id)
    {
        ...
        $this->ViewData['modulePath']   = $this->ModulePath;
        $this->ViewData['object']       = SourceModel::find($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            'name' => 'required|min:1|unique:tbl_source,name,'.$id,
        ],[
            'name.required' => 'Name field is required.',
        ]);

        $arrModel              = SourceModel::find($id);
        $arrModel->name        = $request->name;
        if($arrModel->save())
        {
            $this->JsonData['status'] = 'success';
            $this->JsonData['url']    = url($this->ModulePath.'/edit/'.$id);
            $this->JsonData['msg']    = 'Source details updated successfully';
        }
        else
        {
            $this->JsonData['status'] = 'error';
    ...
    public function destroy($id)
    {
        ...
        {
            $this->JsonData['status'] = 'success';
            $this->JsonData['msg']    = 'Source deleted successfully.';
        }
        else
        {
            $this->JsonData['status'] = 'error';
    ...
    {
        $arrBaseModel = SourceModel::find($id);
