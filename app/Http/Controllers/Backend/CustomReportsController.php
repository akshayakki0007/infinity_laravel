...
class CustomReportsController extends Controller
{
    ...
    {       
        ...
        $this->ModuleView  = 'Backend/Custom_reports.';
        $this->ModulePath  = 'admin/custom_reports_download ';
    }
    ...
    public function getData(Request $request)
    {
        ...
        $modelQuery = $modelQuery->where(function ($query) use ($request){
                        $query->where('fk_category_id',$request->category);
                        $query->where('fk_publisher_id',$request->publisher);
                    });
    ...
    public function download_reports(Request $request)
    {
        ...
        $modelQuery = new ReportsModel();
        $modelQuery = $modelQuery->with(['category','publisher']);
        $modelQuery = $modelQuery->where(function ($query) use ($request){
                    $query->where('fk_category_id',$request->category);
                    $query->where('fk_publisher_id',$request->publisher);
                });
