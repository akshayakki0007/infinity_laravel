...
class PagesController extends Controller
{
    ...
    {       
        ...
        $this->ModuleTitle = 'Pages';
        $this->ModuleView  = 'Backend/Pages.';
        $this->ModulePath  = 'admin/pages';
    }
    ...
    public function getData(Request $request)
    {
        ...
        {   
            ...
            {                
                ...
                $view = '<a title="View" class="btn btn-default btn-circle" href="'.url($row->slug).'"><i class="fa fa-eye" aria-hidden="true"></i></a>&nbsp;';
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            'name'        => 'required|min:1|unique:tbl_pages,name',
            'slug'        => 'required|min:1',
        ...
        $arrModel                    = new PagesModel();
        $arrModel->name              = $request->name;
        $arrModel->slug              = $request->slug;
        $arrModel->contents          = $request->description;
        $arrModel->meta_tite         = $request->meta_tite;
        $arrModel->meta_keyword      = $request->meta_keyword;
        $arrModel->meta_descriptions = $request->meta_descriptions;
        $arrModel->status            = '0';
        ...
        if($arrModel->save()) 
        {
            ...
            $this->JsonData['msg']      = 'Page added successfully.';
        }
        else
        {
            DB::rollBack();
            $this->JsonData['status']   = 'error';
            $this->JsonData['msg']      = 'Failed to page add, Something went wrong.';
        }
    ...
    public function edit($id)
    {
        ...
        $this->ViewData['modulePath']   = $this->ModulePath;
        $this->ViewData['object']       = PagesModel::find($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            'name'        => 'required|min:1|unique:tbl_pages,name,'.$id,
            'slug'        => 'required|min:1',
            'description' => 'required|min:1',
        ],[
            'name.required' => 'Name field is required.',
        ...
        if($arrModel->save())
        {
            ...
            $this->JsonData['msg']      = 'Page details updated successfully';
        }
        else
        {
            $this->JsonData['status'] = 'error';
    ...
    public function destroy($id)
    {
        ...
        {
            $this->JsonData['status'] = 'success';
            $this->JsonData['msg']    = 'Page deleted successfully.';
        }
        else
        {
            $this->JsonData['status'] = 'error';
    ...
    {
        $arrBaseModel = PagesModel::find($id);
